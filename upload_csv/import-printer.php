<?php
    ...
    if (move_uploaded_file($_FILES["file_excel"]["tmp_name"], $target_file)) {
        ...
        for ($i=1; $i <= $numRows ; $i++) {
            $sql = "INSERT INTO printer (merk,type , sn , aktiva , nilai_aktiva , tahun_p , biaya_p , posisi , status , ket , tgl_update , kode )
            VALUES ('".$worksheet[$i]['A']."','".$worksheet[$i]['B']."','".$worksheet[$i]['C']."','".$worksheet[$i]['D']."','".$worksheet[$i]['E']."','".$worksheet[$i]['F']."','".$worksheet[$i]['G']."','".$worksheet[$i]['H']."','".$worksheet[$i]['I']."' ,'".$worksheet[$i]['J']."','".$worksheet[$i]['K']."','".$worksheet[$i]['L']."')";
