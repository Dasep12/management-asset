...
<span>Beranda / Informasi / Web DC </span>
 <li>
<?php
  include 'waktu.php';
?>
...
<div id="container">
  ...
  <a href="add-web.php" class="btn btn-danger" ><i class="fa fa-plus"></i> Tambah Data</a>
                                <table width="100%" class="table-responsive" id="dataTables" >
                                    <thead>
                                        <th>No</th>
                                        <th>Nama WEB</th>
                                        <th>LINK</th>
                    ...
                    include ('koneksi/koneksi.php');
                      $no = 1;
                      $result = mysqli_query($koneksi, "SELECT * FROM web_dc ORDER BY link  ");
                      while($row = mysqli_fetch_array($result)) {?>
                                        <tr class="">
                                            <td><?= $no++ ?></td>
                                            <td><?php echo $row['web'];?></td>
                                            <td><a href="<?php echo $row['link'];?>" target="_new"><?php echo $row['link'];?></a></td>
